...
import { FileText, Plus, Save, Trash2, Edit, Search, X, Loader2 } from "lucide-react";

type Account = { id: string; accountNumber: string; name: string; type: string };
  ...
  return (
    <SitePageShell maxWidth="5xl" subheader={
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Журнал</p>
      ...
      {showForm && (
        <form onSubmit={handleSave} className="mb-6 rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="mb-3 flex items-center justify-between">
            ...
            <button type="button" onClick={() => setLines([...lines, emptyLine()])} className="flex items-center gap-1 text-xs text-emerald-600 hover:text-emerald-800">
                  ...
                  <tr key={line.id}>
                    <td className="p-2">
                      <select value={line.entryType} onChange={(e) => updateLine(line.id, "entryType", e.target.value)}
                        className="w-full rounded border border-slate-300 bg-transparent px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-emerald-500 dark:border-slate-700 dark:text-white">
                    ...
                    <td className="p-2">
                      <input type="number" step="0.01" value={line.amount || ""} onChange={(e) => updateLine(line.id, "amount", Number(e.target.value))}
                        className="w-full rounded border border-slate-300 bg-transparent px-2 py-1 text-right text-xs outline-none focus:ring-1 focus:ring-emerald-500 dark:border-slate-700 dark:text-white" />
                    </td>
                    <td className="p-2">
                      {lines.length > 2 && (
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            <div className="flex gap-4 text-xs text-slate-600 dark:text-slate-400">
              ...
              <span>Баланс: <strong className={isBalanced ? "text-emerald-600" : "text-red-500"}>{isBalanced ? "✓" : `${(totalDebit - totalCredit).toFixed(2)} €`}</strong></span>
            </div>
            <div className="flex gap-2">
              ...
              <button type="submit" disabled={!isBalanced || saving} className="flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700 disabled:opacity-50 dark:bg-white dark:text-slate-950">
                    ...
                    <tr key={e.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="p-3">
                        <div className="flex justify-end gap-2">
                          {e.status === "draft" && <button onClick={() => handlePost(e.id)} className="text-xs text-emerald-600 hover:text-emerald-800 font-bold">Осчетоводи</button>}
