...
- **`agri-page-bg`** — фон на страницата с радиални „петна“ + лек горен ореол за дълбочина. В **`.dark`** задължително се задава `background-color: var(--bg-paper)` (Deep Navy), иначе остава светлият `#F7FAFC` от light и контрастът с `dark:text-*` се чупи.
