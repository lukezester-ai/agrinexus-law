...
const siteUrlRaw = clean(process.env.NEXT_PUBLIC_SITE_URL);
const siteUrl = resolveUrl(siteUrlRaw);
...
console.log("[build-env] site url ok:", Boolean(siteUrl), siteUrl ? `(${siteUrl})` : siteUrlRaw ? `(invalid: ${siteUrlRaw})` : "(default agrinexuslaw.com)");
...
if (process.env.NEXT_PUBLIC_SITE_URL && !siteUrl) {
	console.warn(
		"[build-env] WARNING: NEXT_PUBLIC_SITE_URL is set but invalid — using https://www.agrinexuslaw.com fallback.",
	);
}
