...
import { CalendarDays, Command, FileSearch, Home, MessageCircle } from "lucide-react";

const items = [
	{ href: "/", label: "Начало", Icon: Home },
	{ href: "/search", label: "Документи", Icon: FileSearch },
	{ label: "Команди", Icon: Command, command: true },
...
export function MobileActionDock() {
	const openCommandPalette = () => {
		window.dispatchEvent(new Event("agrinexus:open-command-palette"));
	};
		...
		>
			<div className="grid grid-cols-5 gap-1">
				{items.map((item) => {
					...
					const className =
						"group flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-1.5 py-2 text-[10px] font-semibold text-slate-600 transition hover:bg-emerald-50 hover:text-emerald-800 active:scale-95 dark:text-slate-300 dark:hover:bg-emerald-950/45 dark:hover:text-emerald-200";
					const content = (
						<>
					...
					if ("command" in item) {
						return (
							<button key={item.label} type="button" onClick={openCommandPalette} className={className}>
								{content}
							</button>
						);
					}
